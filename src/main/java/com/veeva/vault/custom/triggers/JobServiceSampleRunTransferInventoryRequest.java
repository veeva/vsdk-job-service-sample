            ...
            recordTriggerContext.getRecordChanges().forEach(rc -> {
                ...
                if (rc.getNew().getValue("transfer_inventory_request_status__c", ValueType.PICKLIST_VALUES).contains("pending__c")) {
                    JobQueueParameters jobQueueParameters = jobService.newJobQueueParametersBuilder()
                            .appendParameter("newBicycleStoreId", rc.getNew().getValue("new_bicycle_store__c", ValueType.STRING))
                            .appendParameter("existingBicycleStoreId", rc.getNew().getValue("existing_bicycle_store__c", ValueType.STRING))
                            .build();
                    JobQueueRequest jobQueueRequest = jobService.newJobQueueRequestBuilder()
                            .withJobName("transfer_product_to_new_store__c")
